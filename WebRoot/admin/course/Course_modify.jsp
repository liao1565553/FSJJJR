...
<body>
	...
	<div id="mainContainer">
		...
		<form name="modifyForm"
			action="<%=path%>/UpLoad.action?types=course&operation=save"
			method="post" enctype="multipart/form-data">
			<table width="750px">
				<tr>
					<td width="12%">课程编号：</td>
					<td><input type="text" name="id" readonly="readonly"
						value='<s:property value="#attr.modify_course.id"/>' /></td>
				</tr>
				<tr>
					<td width="12%">课程标题：</td>
					<td><input type="text" name="title"
						value='<s:property value="#attr.modify_course.title"/>' /></td>
				</tr>
				<tr>
					<td width="12%">课程类别：</td>
					<td><SELECT NAME="type" size=1>
							<c:forEach items="${Type_list}" var="tl">
								<c:if test="${tl.tid==modify_course.tp.tid}">
				...
				<tr>
					...
					<td><input name="datetime" type="text"
						value="<s:date name="#attr.modify_course.datetime" format="yyyy-MM-dd"/>" /></td>
				</tr>
				<tr>
					<td>上传图片：</td>
