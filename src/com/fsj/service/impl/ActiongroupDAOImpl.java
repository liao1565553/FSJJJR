...
//权限分栏业务逻辑接口的实现类
public class ActiongroupDAOImpl implements ActiongroupDAO {
	@Override
	...
	public List<Actiongroup> querypage(int pageNumber, int pageSize,
		...
		// TODO Auto-generated method stub
		Transaction tx = null;
		List<Actiongroup> listpage = null;
		try {
			// 获得会话
			Session session = MyHibernateSessionFactory.getSessionFactory()
					.getCurrentSession();
	...
	public boolean deleteActiongroup(int agid) {
		...
		try {
			...
			tx = session.beginTransaction();
			// 获得指定编号的权限分栏
			Actiongroup actiongroup = (Actiongroup) session.get(
					Actiongroup.class, agid);
			// 删除该权限分栏
			session.delete(actiongroup);
			// 提交事物
			tx.commit();
			return true;
		} catch (Exception ex) {
